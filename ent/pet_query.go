...
// QueryOwningUser chains the current query on the "owningUser" edge.
func (pq *PetQuery) QueryOwningUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, pet.OwningUserTable, pet.OwningUserColumn),
		)
...
func (pq *PetQuery) Clone() *PetQuery {
	...
	return &PetQuery{
		...
		withOwningUser: pq.withOwningUser.Clone(),
...
// WithOwningUser tells the query-builder to eager-load the nodes that are connected to
// the "owningUser" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PetQuery) WithOwningUser(opts ...func(*UserQuery)) *PetQuery {
	query := &UserQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withOwningUser = query
	return pq
}
...
func (pq *PetQuery) sqlAll(ctx context.Context) ([]*Pet, error) {
	...
	if query := pq.withOwningUser; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "owningUserID" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.OwningUser = n
			}
		}
	}
